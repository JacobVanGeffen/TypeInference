...
void SymbolTable::print_contents() {
	...
	int i = 0;
	for(vector<map<AstIdentifier*, Expression*> >::iterator it = values.begin(); it != values.end(); it++, i++) {
		cout << "Bindings at level " << i <<  ":" << endl;
		map<AstIdentifier*, Expression*> & cur = *it;
		for(map<AstIdentifier*, Expression*>::iterator it2 = cur.begin(); it2 != cur.end(); it2++) {
