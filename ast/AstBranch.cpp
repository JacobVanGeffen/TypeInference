...
AstBranch* AstBranch::make(Expression* pred, Expression* then_exp, Expression* else_exp) {
...
AstBranch::AstBranch(Expression* pred, Expression* then_exp, Expression* else_exp) {
	...
	this->hash_c = 5 * pred->get_hash() + 2 * then_exp->get_hash() + 732 * else_exp->get_hash();
}
...
string AstBranch::to_value() {
	return "(if " + pred->to_value() + " then " + then_exp->to_value() + " else " + else_exp->to_value() + ")";
}
...
Expression* AstBranch::substitute(Expression* e1, Expression* e2) {
	...
	if(pred == new_pred && then_exp == new_then && else_exp == new_else) return this;
